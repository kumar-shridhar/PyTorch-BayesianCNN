...
class BBBConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, 
        stride, padding, dilation=1, output_padding=0, groups=1, p_logvar_init, p_pi=1.0, q_logvar_init):
...
class BBBLinearFactorial(nn.Module):
    ...
    def __init__(self, in_features, out_features, p_logvar_init, p_pi=1.0, q_logvar_init):
    ...
    def fcprobforward(self, input):
        ...
        sigma_weight = torch.exp(self.fc_qw_std)
        weight = self.fc_qw_mean + sigma_weight * self.eps_weight.normal_()
